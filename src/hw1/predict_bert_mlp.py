...
import matplotlib
from models.transmlp import AllDataset, TransMLP
...
if __name__ == "__main__":
    ...
    train_loader=DataLoader(dataset=AllDataset('train', 0.8, train_set, train_label), batch_size=32, shuffle=True)
    test_loader =DataLoader(dataset=AllDataset('test', 0.8, test_set, test_label),   batch_size=32, shuffle=False)
    ...
    for epoch in range(500):
        ...
        for idx, (content, ori, label) in enumerate(train_loader):
            sentences_tokenizer = tokenizer(content[0], truncation=True, padding=True, max_length=512, add_special_tokens=True)
            ...
            ori = ori[0].squeeze(1).to(device)
            label = label[0].to(device)
            pred = model(ori, input_ids, attention_mask)
